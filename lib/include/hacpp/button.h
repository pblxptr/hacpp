      ...
      res->visit([&](auto &&packet) {
        ...
        if constexpr (std::is_same_v<PacketType,
          ...
          if (packet.topic() == config_.cfg[Opt::CommandTopic] &&
            ...
            if (config_.handler) {
              boost::asio::co_spawn(client_.executor(), config_.handler(),
                                    boost::asio::detached);
            }
          }
        }
...
public:
  ...
  auto create() {
    return Button{Button::Config{.unique_id = unique_id_,
                                 .qos = qos_,
                                 .cfg = cfg_,
...
private:
  ...
  EntityCfg cfg_{
      ...
      {Button::Opt::CommandTopic,
       default_component_command_topic(Button::Defs::Component, unique_id_)}};
