...
public:
  ...
  struct Opt {
    ...
    constexpr static Property PayloadOpen{"payload_open"};
    constexpr static Property PayloadClose{"payload_close"};
    constexpr static Property PayloadStop{"payload_stop"};
    constexpr static Property StateOpen{"state_open"};
    constexpr static Property StateClosed{"state_closed"};
    constexpr static Property StateOpening{"state_opening"};
    constexpr static Property StateClosing{"state_closing"};
    constexpr static Property Device{"device"};
  };

  struct Defs {
    constexpr static auto Component = "cover";
    constexpr static auto PayloadOpen = "OPEN";
    constexpr static auto PayloadClose = "CLOSE";
    constexpr static auto PayloadStop = "STOP";
    constexpr static auto StateOpen = "open";
    constexpr static auto StateClosed = "closed";
    constexpr static auto StateOpening = "opening";
    constexpr static auto StateClosing = "closing";
  };
  ...
  const EntityCfg &config() const { return config_.cfg; }
  ...
  boost::asio::awaitable<Error> async_update_availability(bool state) {
    ...
    co_return co_await client_.async_publish(
        config_.cfg[Availability::Opt::Topic], val, config_.qos);
  }
        ...
        if constexpr (std::is_same_v<PacketType,
          ...
          if (packet.topic() == config_.cfg[Opt::CommandTopic]) {
            ...
            if (payload == config_.cfg[Opt::PayloadOpen]) {
              if (config_.on_open) {
                boost::asio::co_spawn(client_.executor(), config_.on_open(),
                                      boost::asio::detached);
              }
            } else if (payload == config_.cfg[Opt::PayloadClose]) {
              if (config_.on_close) {
                boost::asio::co_spawn(client_.executor(), config_.on_close(),
                                      boost::asio::detached);
              }
            } else if (payload == config_.cfg[Opt::PayloadStop]) {
              if (config_.on_stop) {
                boost::asio::co_spawn(client_.executor(), config_.on_stop(),
                                      boost::asio::detached);
              }
            }
          }
  ...
  boost::asio::awaitable<void> async_close() { co_await client_.async_close(); }
...
template <> class Factory<Cover> {
public:
  Factory(const std::string &unique_id, ClientType client)
      : unique_id_(unique_id), client_(std::move(client)) {}

  template <typename T, typename V> auto &set(T &&key, V &&value) {
  ...
  auto create() {
    return Cover{Cover::Config{.unique_id = unique_id_,
                               .qos = qos_,
                               .cfg = cfg_,
...
private:
  ...
  EntityCfg cfg_{
      {Cover::Opt::CommandTopic,
       default_component_command_topic(Cover::Defs::Component, unique_id_)},
