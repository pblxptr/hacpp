...
get_verifier(boost::asio::any_io_executor exe) {
  ...
  auto sub_topics = std::vector<TopicSubopts>{
      {default_component_discovery_topic(Sensor::Defs::Component, UniqueId),
       QoS::at_most_once},
      {default_component_state_topic(Sensor::Defs::Component, UniqueId),
       QoS::at_most_once},
      {default_component_availability_topic(Sensor::Defs::Component, UniqueId),
       QoS::at_most_once}};
...
TEST_CASE("Sensor provides all required options during discovery", "[sensor]") {
  ...
  boost::asio::co_spawn(
      ...
      [&]() mutable -> boost::asio::awaitable<void> {
        ...
        REQUIRE(
            !pobj.as_object()[Sensor::Opt::StateTopic.key].as_string().empty());
  ...
  boost::asio::co_spawn(
      ...
      [&]() mutable -> boost::asio::awaitable<void> {
        ...
        SECTION("provides 'online' state") {
          ...
          // Assert
          REQUIRE(!err);
          REQUIRE(packet.topic() == default_component_availability_topic(
                                        Sensor::Defs::Component, UniqueId));
