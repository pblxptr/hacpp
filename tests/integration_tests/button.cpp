...
get_verifier(boost::asio::any_io_executor exe) {
  ...
  auto sub_topics = std::vector<TopicSubopts>{
      {default_component_discovery_topic(Button::Defs::Component, UniqueId),
       QoS::at_most_once},
      {default_component_command_topic(Button::Defs::Component, UniqueId),
       QoS::at_most_once},
      {default_component_availability_topic(Button::Defs::Component, UniqueId),
       QoS::at_most_once}};
...
TEST_CASE("Button provides all required options during discovery", "[button]") {
  ...
  boost::asio::co_spawn(
      ...
      [&]() mutable -> boost::asio::awaitable<void> {
        ...
        // clang-format off
        auto button = Factory<Button>(UniqueId, std::move(entity_client)).create();
        // clang-format on
        // Act
        ...
        REQUIRE(!pobj.as_object()[Button::Opt::CommandTopic.key]
                     .as_string()
                     .empty());
