...
static boost::asio::awaitable<ClientType>
get_client(boost::asio::any_io_executor exe) {
...
static boost::asio::awaitable<ClientType>
get_verifier(boost::asio::any_io_executor exe) {
  auto client = ClientType{exe, config};
  auto err = co_await client.async_connect();
  REQUIRE(!err);

  auto sub_topics = std::vector<TopicSubopts>{
      {default_component_discovery_topic(BinarySensor::Defs::Component,
                                         UniqueId),
       QoS::at_most_once},
      {default_component_state_topic(BinarySensor::Defs::Component, UniqueId),
       QoS::at_most_once},
      {default_component_availability_topic(BinarySensor::Defs::Component,
                                            UniqueId),
       QoS::at_most_once}};
...
TEST_CASE("Binary sensor provides all required options during discovery", "[binary_sensor]") {
  ...
  boost::asio::co_spawn(
      ...
      [&]() mutable -> boost::asio::awaitable<void> {
        ...
        // Assert
        REQUIRE(!err);
        REQUIRE(packet.topic() == default_component_discovery_topic(
        ...
        REQUIRE(!pobj.as_object()[BinarySensor::Opt::StateTopic.key]
                     .as_string()
                     .empty());
  ...
  boost::asio::co_spawn(
      ...
      [&]() mutable -> boost::asio::awaitable<void> {
        ...
        SECTION("provides 'on' state") {
          ...
          // Assert
          REQUIRE(!err);
          REQUIRE(packet.topic() ==
                  binary_sensor.config().at(BinarySensor::Opt::StateTopic));
