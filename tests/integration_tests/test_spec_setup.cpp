...
void test_spec_setup() {
  ...
  config.add_option(
      TestOption{.name = MqttUsernameOptionName,
                 .hint = "mqtt client username",
                 .description = "MQTT client username eg. test_client_id"});

  config.add_option(
      TestOption{.name = MqttPasswordOptionName,
  ...
  config.add_option(TestOption{.name = MqttServerPortOptionName,
                               .hint = "mqtt server port",
                               .description = "MQTT server port, eg. 8123"});

  config.add_option(TestOption{.name = MqttUniqueIdOptionName,
